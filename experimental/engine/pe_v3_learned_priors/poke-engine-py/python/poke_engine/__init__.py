...
def shared_information_set_root_search(
    ...
    When ``baseline_action`` is supplied, the diagnostics compare the shared
    policy and baseline on paired cached MCTS payoff cells. Those values are
    biased estimates, so the reported LCB is an empirical heuristic rather
    than a formal confidence guarantee.
    """
