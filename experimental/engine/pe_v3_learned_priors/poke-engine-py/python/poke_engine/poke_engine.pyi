...
def mcts_leaf_counterfactual_sample(
    ...
    c_puct: float = 2.0,
    rollout_iterations: int = 1000,
    max_decisions: int = 32,
) -> Optional[Tuple[List[float], List[float], int, int, int, Optional[float]]]: ...
def mcts_leaf_state_sample(
    py_state: State,
    duration_ms: int = 1000,
    iterations: int = 0,
    s1_priors: Optional[List[Tuple[str, float]]] = None,
    s2_priors: Optional[List[Tuple[str, float]]] = None,
    c_puct: float = 2.0,
) -> Optional[Tuple[List[float], List[float], str, int, int, int]]: ...
def mcts_rollout_to_terminal(
    py_state: State,
    rollout_iterations: int = 1000,
    max_decisions: int = 32,
) -> Optional[float]: ...
