...
impl Into<State> for PyState {
    fn into(self) -> State {
        let mut state = State {
            ...
            threat_matrix: {
                let mut m = [0.0f32; 36];
                for (i, v) in self.threat_matrix.iter().enumerate().take(36) {
                    m[i] = *v;
                }
                m
            },
            wincon_matrix: {
                let mut m = [0.0f32; 36];
                for (i, v) in self.wincon_matrix.iter().enumerate().take(36) {
                    m[i] = *v;
                }
                m
            },
...
#[derive(Clone)]
#[pyclass(name = "SharedRootPolicyEntry", get_all)]
...
#[derive(Clone)]
#[pyclass(name = "SharedRootDiagnostics", get_all)]
struct PySharedRootDiagnostics {
    rounds: u32,
    continuation_iterations: u32,
    unique_payoff_cells_evaluated: u64,
    cache_hits: u64,
    total_forced_continuation_iterations: u64,
    world_pulls: Vec<u32>,
    elapsed_ms: u64,
    shared_policy_entropy: f32,
    shared_policy_max_probability: f32,
    human_prior_mix: f32,
    player_prior_mix: f32,
    player_prior_available: bool,
    player_prior_coverage: f32,
    baseline_action: Option<String>,
    baseline_advantage_available: bool,
    baseline_advantage_mean: Option<f32>,
    baseline_advantage_standard_error: Option<f32>,
    baseline_advantage_lcb: Option<f32>,
    baseline_advantage_world_count: u32,
...
#[derive(Clone)]
#[pyclass(name = "SharedInformationSetRootResult", get_all)]
    ...
    fn from(result: SharedRootResult) -> Self {
        Self {
            ...
            diagnostics: PySharedRootDiagnostics {
                ...
                total_forced_continuation_iterations: result
                    .diagnostics
                ...
                baseline_advantage_standard_error: result
                    .diagnostics
                    .baseline_advantage_standard_error,
                ...
                baseline_advantage_effective_world_count: result
                    .diagnostics
...
fn match_priors_to_options(
    ...
    Some(matched.into_iter().map(|v| Some(v / total)).collect())
}
...
#[pyo3(signature = (states, world_weights, duration_ms=0, rounds=0, continuation_iterations=32, s1_prior=None, player_prior_mix=0.25, s2_priors=None, human_prior_mix=0.25, min_policy_probability=0.02, seed=0, baseline_action=None, lcb_z=1.645, paired_evaluation_iterations=512))]
fn py_shared_information_set_root_search(
    states: Vec<PyState>,
    world_weights: Vec<f64>,
    duration_ms: u64,
    rounds: u32,
    continuation_iterations: u32,
    ...
    engine_shared_information_set_root_search(
        &states,
        &world_weights,
...
#[pyfunction]
#[pyo3(signature = (py_state, duration_ms=1000, iterations=0, s1_priors=None, s2_priors=None, c_puct=2.0))]
fn mcts_leaf_sample(
    py_state: PyState,
    duration_ms: u64,
    iterations: u32,
    ...
    s2_priors: Option<Vec<(String, f32)>>,
    c_puct: f32,
) -> PyResult<Option<(Vec<f32>, Vec<f32>, f32, u8, u32, u32)>> {
    let mut state: State = py_state.into();
    let (s1_options, s2_options) = state.root_get_all_options();
    let s1_priors_vec = match_priors_to_options(&s1_priors, &s1_options, &state.side_one);
    let s2_priors_vec = match_priors_to_options(&s2_priors, &s2_options, &state.side_two);
    ...
    Ok(result.leaf_sample.map(|sample| {
        (
            sample.root_features,
            sample.leaf_features,
            sample.target,
            sample.depth,
            sample.terminal_visits,
            sample.all_visits,
        )
    }))
}
...
#[pyfunction]
#[pyo3(signature = (py_state, duration_ms=1000, iterations=0, s1_priors=None, s2_priors=None, c_puct=2.0, rollout_iterations=1000, max_decisions=32))]
fn mcts_leaf_counterfactual_sample(
    py_state: PyState,
    duration_ms: u64,
    iterations: u32,
    s1_priors: Option<Vec<(String, f32)>>,
    ...
    max_decisions: u32,
) -> PyResult<Option<(Vec<f32>, Vec<f32>, u8, u32, u32, Option<f32>)>> {
    let mut state: State = py_state.into();
    let (s1_options, s2_options) = state.root_get_all_options();
    let s1_priors_vec = match_priors_to_options(&s1_priors, &s1_options, &state.side_one);
    let s2_priors_vec = match_priors_to_options(&s2_priors, &s2_options, &state.side_two);
    ...
    Ok(result.leaf_sample.map(|sample| {
        let outcome =
            rollout_leaf_to_terminal(sample.sampled_state, rollout_iterations, max_decisions);
        (
            sample.root_features,
            sample.leaf_features,
            sample.depth,
...
fn mcts_leaf_state_sample(
    ...
    s2_priors: Option<Vec<(String, f32)>>,
    c_puct: f32,
) -> PyResult<Option<(Vec<f32>, Vec<f32>, String, u8, u32, u32)>> {
    let mut state: State = py_state.into();
    let (s1_options, s2_options) = state.root_get_all_options();
    let s1_priors_vec = match_priors_to_options(&s1_priors, &s1_options, &state.side_one);
    let s2_priors_vec = match_priors_to_options(&s2_priors, &s2_options, &state.side_two);
    ...
    Ok(result.leaf_sample.map(|sample| {
        (
            sample.root_features,
            sample.leaf_features,
            sample.sampled_state.serialize(),
            sample.depth,
            sample.terminal_visits,
            sample.all_visits,
        )
...
#[pyfunction]
#[pyo3(signature = (py_state, rollout_iterations=1000, max_decisions=32))]
fn mcts_rollout_to_terminal(
    py_state: PyState,
    rollout_iterations: u32,
    max_decisions: u32,
) -> PyResult<Option<f32>> {
    let state: State = py_state.into();
    Ok(rollout_leaf_to_terminal(
        state,
        rollout_iterations,
        max_decisions,
    ))
...
#[pyfunction]
fn set_belief(
    py_state: &Bound<'_, PyState>,
    s1_threat: f32,
    s2_threat: f32,
    scout_value: f32,
) -> PyResult<()> {
    let mut st = py_state.borrow_mut();
    st.s1_threat = s1_threat;
    st.s2_threat = s2_threat;
    st.scout_value = scout_value;
    Ok(())
}

#[pyfunction]
fn set_threat_matrix(py_state: &Bound<'_, PyState>, flat_matrix: Vec<f32>) -> PyResult<()> {
    if flat_matrix.len() != 36 {
        return Err(pyo3::exceptions::PyValueError::new_err(
            "flat_matrix must have exactly 36 elements (6x6 row-major)",
        ));
    }
    let mut st = py_state.borrow_mut();
    st.threat_matrix = flat_matrix;
    Ok(())
}

#[pyfunction]
fn set_wincon_matrix(py_state: &Bound<'_, PyState>, flat_matrix: Vec<f32>) -> PyResult<()> {
    if flat_matrix.len() != 36 {
        return Err(pyo3::exceptions::PyValueError::new_err(
            "flat_matrix must have exactly 36 elements (6x6 row-major)",
        ));
...
#[pyfunction]
fn compute_value_features(state: PyState) -> Vec<f32> {
    let s: poke_engine::state::State = state.into();
