...
def load_counterfactual_examples(paths: list[Path]):
    features, targets, groups, skipped = [], [], [], 0
    for path in paths:
        for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
            try:
                row = json.loads(line)
                target = row["target"]
                feature = row["leaf_features"]
            except (json.JSONDecodeError, KeyError, TypeError):
                skipped += 1
                continue
            if target not in (-1.0, 1.0) or len(feature) != 14:
                skipped += 1
                continue
            features.append(feature)
            targets.append((target + 1.0) / 2.0)
            groups.append((str(row.get("battle_tag")), str(row.get("username"))))
    if not features:
        raise ValueError("no complete, feature-valid counterfactual samples")
    return features, targets, groups, {"invalid_or_incomplete": skipped}


def main():
    parser = argparse.ArgumentParser()
    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--decision-log", type=Path, action="append")
    source.add_argument("--counterfactual-sample", type=Path, action="append")
    ...
    if args.counterfactual_sample:
        x, y, groups, skipped = load_counterfactual_examples(args.counterfactual_sample)
    else:
