#!/usr/bin/env python3
"""Verify every frozen Cycle30 dynamic-boundary smoke input."""
...
def verify(path: Path) -> dict:
    ...
    if payload.get("schema") != "metagross-cycle30-dynamic-boundary-presmoke-freeze/v1":
        raise RuntimeError("wrong Cycle30 presmoke manifest schema")
    for row in payload["files"]:
        source = Path(row["path"])
        if not source.is_file() or sha(source) != row["sha256"]:
            raise RuntimeError(f"Cycle30 frozen file mismatch: {source}")
    ...
    if commit != showdown["commit"] or tree_sha256(root / "dist") != showdown["dist_tree_sha256"]:
        raise RuntimeError("Cycle30 Showdown runtime changed")
    return payload
