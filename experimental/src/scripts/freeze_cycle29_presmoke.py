#!/usr/bin/env python3
"""Freeze Cycle29 before its fresh two-decision operational smoke."""
...
from experimental.src.scripts.cycle29_canonical_smoke import config_identity
...
PAIR_SHA = "a072098fc7dbc6039ba50b846783d70c86a184c01d2230ab0a5f4f8618ba873f"
...
def main() -> None:
    ...
    if output.exists():
        raise RuntimeError("Cycle29 smoke is already frozen")
    ...
    if (
        ...
        raise RuntimeError("Cycle29 mechanics did not authorize this smoke")
    tests = json.loads((RUN / "PRESMOKE_TESTS.json").read_text())
    if tests.get("status") != "pass" or tests.get("passed") != 20:
        raise RuntimeError("Cycle29 presmoke tests did not pass")
    pair = RUN / "smoke-result.json.pairs.json"
    if sha(pair) != PAIR_SHA:
        raise RuntimeError("Cycle29 fresh pair changed")
    payload = json.loads(pair.read_text())
    if len(payload.get("pairs", [])) != 1:
        raise RuntimeError("Cycle29 smoke must contain one fresh pair")
    row = payload["pairs"][0]
    ...
    if current in prior:
        raise RuntimeError("Cycle29 smoke pair overlaps a prior pair")
    ...
    if prepare != live or prepare != payload["config_sha256"]:
        raise RuntimeError("Cycle29 preparation/live config identity changed")
    registration_dir = RUN / "smoke-registrations"
    if not registration_dir.is_dir() or any(registration_dir.iterdir()):
        raise RuntimeError("Cycle29 registration domain is not fresh and empty")
    ...
    if sha(extension) != ENGINE_SHA:
        raise RuntimeError("Cycle29 pinned engine changed")
    files = [
        RUN / "PROTOCOL.md",
        RUN / "PREMEASUREMENT_MANIFEST.json",
        RUN / "mechanics-audit/REPORT.json",
        RUN / "mechanics-audit/fixture-results.jsonl",
        RUN / "selection-16.jsonl",
        RUN / "PRESMOKE_TESTS.json",
        RUN / "CANONICAL_SMOKE_ARGV.json",
        RUN / "SHOWDOWN_RUNTIME_MANIFEST.json",
        pair,
        ROOT / "experimental/src/scripts/cycle29_canonical_smoke.py",
        ROOT / "experimental/src/scripts/watch_cycle29_registrations.py",
        ROOT / "experimental/src/scripts/monitor_cycle29_second_root_smoke.py",
        ROOT / "experimental/src/scripts/run_cycle29_second_root_smoke.sh",
        ROOT / "experimental/src/scripts/freeze_cycle29_presmoke.py",
        ROOT / "experimental/src/scripts/verify_cycle29_presmoke.py",
    ...
    manifest = {
        "schema": "metagross-cycle29-second-root-presmoke-freeze/v1",
        "status": "frozen_before_fresh_second_root_smoke",
        "protocol_sha256": sha(RUN / "PROTOCOL.md"),
        "mechanics_manifest_sha256": sha(RUN / "PREMEASUREMENT_MANIFEST.json"),
        ...
        "gate": {
            ...
            "opening_decision_index": 0,
            "opening_production_and_candidate_receipts_empty": True,
