...
PAIR_SHA="08a2becc4a5266bf4e78bd885d43a771b4db42c53d06a32dcc8479be351ea830"
...
if [[ "${METAGROSS_CYCLE25_EXECUTE:-}" != "FROZEN_ATTRIBUTED_FIRST_DECISION_SMOKE" ]]; then
  echo "set METAGROSS_CYCLE25_EXECUTE=FROZEN_ATTRIBUTED_FIRST_DECISION_SMOKE" >&2; exit 2
fi
PYTHONPATH="$ROOT/experimental/src:$ROOT" "$ROOT/.venv-metamon/bin/python" \
  "$ROOT/experimental/src/scripts/verify_cycle25_presmoke.py" "$MANIFEST" >/dev/null
PYTHONPATH="$ROOT/experimental/src:$ROOT" "$ROOT/.venv-metamon/bin/python" \
  "$ROOT/experimental/src/scripts/cycle25_canonical_eval.py" --canonical "$CANONICAL" \
...
[[ ! -e "$RUN/SMOKE_RESULT.json" && ! -e "$RUN/REGISTRATION_CONSUMPTION.json" \
  ...
  echo "Cycle25 live outputs already exist" >&2; exit 2;
}
...
cleanup() {
  ...
  while read -r pid; do [[ -n "$pid" ]] && kill "$pid" 2>/dev/null || true; done \
    < <(pgrep -f 'srcs.metagross.run_foul_play.*c25smk' || true)
}
...
for side in a b; do
  ...
  env METAMON_CACHE_DIR="$ROOT/external/metamon_cache" TORCHDYNAMO_DISABLE=1 ACCELERATE_USE_CPU=true CUDA_VISIBLE_DEVICES='' \
    ...
    --username "c25smk${side}" --port "$port" --decision-dump "$RUN/prior-${side}.jsonl" \
...
PYTHONPATH="$ROOT/experimental/src:$ROOT" "$ROOT/.venv-metamon/bin/python" -u \
  "$ROOT/experimental/src/scripts/watch_cycle25_registrations.py" \
...
env METAGROSS_CYCLE25_BOUNDARY_RECEIPT="$RUN/PUBLIC_EXECUTION_BOUNDARY.json" METAMON_CACHE_DIR="$ROOT/external/metamon_cache" TORCHDYNAMO_DISABLE=1 ACCELERATE_USE_CPU=true CUDA_VISIBLE_DEVICES='' \
...
PYTHONPATH="$ENGINE_ROOT:$ROOT/srcs/vendor/foul-play:$ROOT/experimental/src:$ROOT" \
  "$ROOT/.venv-metamon/bin/python" "$ROOT/experimental/src/scripts/monitor_cycle25_attributed_smoke.py" \
