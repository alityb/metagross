#!/usr/bin/env python3
"""Derive Cycle32 prepare/live smoke argv from one canonical object."""
...
def load_canonical(path: Path) -> tuple[Path, list[str]]:
    ...
    if payload.get("schema") != "metagross-cycle32-canonical-smoke-argv/v1":
        raise RuntimeError("wrong Cycle32 canonical argv schema")
    evaluator, argv = Path(payload["evaluator"]).resolve(), payload.get("argv")
    if evaluator != (ROOT / "experimental/src/eval/run.py").resolve():
        raise RuntimeError("Cycle32 evaluator path changed")
    if not isinstance(argv, list) or not argv or any(not isinstance(x, str) for x in argv):
        raise RuntimeError("Cycle32 canonical argv is malformed")
    if {"--prepare-mirrored-pairs-only", "--pair-manifest-sha256"}.intersection(argv):
        raise RuntimeError("Cycle32 canonical argv contains projected fields")
    return evaluator, argv


def derived_argv(canonical: Path, phase: str, pair_sha256: str | None = None):
    ...
    if phase == "prepare":
        ...
        if pair_sha256 is None or len(pair_sha256) != 64:
            raise RuntimeError("Cycle32 live phase requires frozen pair SHA")
        derived += ["--pair-manifest-sha256", pair_sha256]
    else:
        raise RuntimeError("unknown Cycle32 phase")
...
def main() -> None:
    ...
    if args.phase == "verify":
        if args.pair_manifest is None or args.pair_sha256 is None:
            raise RuntimeError("Cycle32 verify requires pair manifest and SHA")
        if sha(args.pair_manifest) != args.pair_sha256:
            raise RuntimeError("Cycle32 pair bytes differ from frozen SHA")
        ...
        if prepared != live or prepared != stored:
            raise RuntimeError("Cycle32 preparation/live identity mismatch")
