#!/usr/bin/env python3
"""Freeze Cycle 38b canonical-path replay without changing mechanics."""
...
RUN = ROOT / "experimental/runs/search_native_v2_cycle38b_temporal_request_20260816"
...
def main() -> None:
    if MANIFEST.exists() or not RUN.is_absolute():
        raise SystemExit("Cycle38b manifest exists or run path is noncanonical")
    ...
    paths = [
        ...
        ROOT / "experimental/src/scripts/freeze_cycle38b_temporal_request_lineage.py",
        ...
        ROOT / "experimental/runs/search_native_v2_cycle38_temporal_request_20260816/VOID_REPORT.json",
        ENGINE, Path(shutil.which("node") or ""),
    ]
    paths.extend(Path(row["raw_path"]) for row in selected)
    if any(not path.is_file() for path in paths):
        raise SystemExit("Cycle38b frozen source missing")
    runtimes = []
    for commit in sorted({row["showdown_commit"] for row in selected}):
        ...
        if actual != commit:
            raise SystemExit("Cycle38b Showdown runtime mismatch")
        runtimes.append({
            "commit": commit, "path": str(worktree),
            "dist_tree_sha256": tree_sha256(worktree / "dist"),
        })
    cycle38_manifest = ROOT / (
        "experimental/runs/search_native_v2_cycle38_temporal_request_20260816/"
        "PREMEASUREMENT_MANIFEST.json"
    )
    payload = {
        "schema": "metagross-cycle38b-canonical-path-freeze/v1",
        "protocol_sha256": sha256(RUN / "PROTOCOL.md"),
        "void_cycle38_manifest_sha256": sha256(cycle38_manifest),
    ...
    print(json.dumps({
        ...
        "canonical_absolute_run_dir": payload["canonical_absolute_run_dir"],
