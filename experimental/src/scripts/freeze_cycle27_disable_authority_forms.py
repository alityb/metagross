#!/usr/bin/env python3
"""Freeze Cycle 27 selection, sources, pinned runtimes, and gates."""
...
def main() -> None:
    if MANIFEST.exists():
        raise SystemExit("Cycle27 manifest already exists")
    if sha256(SELECTION) != sha256(SOURCE_SELECTION):
        raise SystemExit("Cycle27 selection is not byte-identical to Cycle14")
    ...
    if (
        ...
        raise SystemExit("Cycle27 selection violates frozen TRAIN/cluster contract")
    ...
    source_paths = [
        ...
        ROOT / "experimental/src/scripts/freeze_cycle27_disable_authority_forms.py",
    ...
    if any(not path.is_file() for path in source_paths):
        raise SystemExit("Cycle27 frozen source is missing")
    files = [
    ...
    for commit in commits:
        ...
        if actual != commit:
            raise SystemExit("Cycle27 Showdown worktree commit mismatch")
        runtimes.append({
            "commit": commit,
            "path": str(worktree),
    ...
    payload = {
        "schema": "metagross-cycle27-disable-authority-manifest/v1",
