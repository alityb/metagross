...
PAIR_SHA="f863fe75621a8617920205014e88330a66a58c7ab857e6d4ed5e6038c1bf8c8c"
...
if [[ "${METAGROSS_CYCLE23_EXECUTE:-}" != "FROZEN_FIRST_DECISION_SMOKE" ]]; then
  echo "set METAGROSS_CYCLE23_EXECUTE=FROZEN_FIRST_DECISION_SMOKE" >&2; exit 2
fi
PYTHONPATH="$ROOT/experimental/src:$ROOT" "$ROOT/.venv-metamon/bin/python" \
  "$ROOT/experimental/src/scripts/verify_cycle23_presmoke.py" "$MANIFEST" >/dev/null
...
[[ ! -e "$RUN/SMOKE_RESULT.json" && ! -e "$RUN/REGISTRATION_CONSUMPTION.json" \
  ...
  echo "Cycle23 live outputs already exist" >&2; exit 2;
}
...
cleanup() {
  ...
  while read -r pid; do [[ -n "$pid" ]] && kill "$pid" 2>/dev/null || true; done \
    < <(pgrep -f 'srcs.metagross.run_foul_play.*c23smk' || true)
}
...
for side in a b; do
  ...
  env METAMON_CACHE_DIR="$ROOT/external/metamon_cache" TORCHDYNAMO_DISABLE=1 ACCELERATE_USE_CPU=true CUDA_VISIBLE_DEVICES='' \
    ...
    --username "c23smk${side}" --port "$port" --decision-dump "$RUN/prior-${side}.jsonl" \
...
env METAMON_CACHE_DIR="$ROOT/external/metamon_cache" TORCHDYNAMO_DISABLE=1 ACCELERATE_USE_CPU=true CUDA_VISIBLE_DEVICES='' \
  ...
  --websocket-uri ws://127.0.0.1:8010/showdown/websocket --paired --mirrored-pairs --mirror-seed 202623000260 \
  ...
  --production-run-seed 2323232323232323232323232323232323232323232323232323232323232323 \
  --concurrent-games 1 --fail-fast --game-timeout-seconds 900 --n-games 2 \
  --username-prefix c23smk --run-id cycle23-first-decision-smoke --pair-manifest-sha256 "$PAIR_SHA" \
...
PYTHONPATH="$ENGINE_ROOT:$ROOT/srcs/vendor/foul-play:$ROOT/experimental/src:$ROOT" \
  "$ROOT/.venv-metamon/bin/python" "$ROOT/experimental/src/scripts/monitor_cycle23_first_decision_smoke.py" \
  --run "$RUN" --pair-manifest "$PAIR" --expected-engine-sha256 "$ENGINE_SHA" \
  --timeout-seconds 300 --output "$RUN/SMOKE_RESULT.json"
# The frozen boundary is the first public execution; stop before a second root.
