#!/usr/bin/env python3
"""Cycle 27 causal move/PP/disable mechanics over frozen TRAIN roots."""
...
WORLD_COUNT = 8
SCHEDULE_COUNT = 2
BASE_SEED = 2026081527


class Cycle27Error(RuntimeError):
    pass
...
def _weight_bytes(worlds: list[tuple[Any, Any]]) -> bytes:
    ...
    for _battle, weight in worlds:
        if not isinstance(weight, (float, int)) or isinstance(weight, bool):
            raise Cycle27Error("sampler weight is not numeric")
...
def _assert_engine_disable_receipts(
    sampled: Any, receipt: Mapping[str, Any], search_main: Any,
) -> None:
    """Prove search receives each world's final PP/disabled mechanics."""
    ...
    for row in receipt["moves"]:
        candidates = slots.get(row["exact_public_species"], [])
        if len(candidates) != 1:
            raise Cycle27Error("disable receipt exact engine slot disagrees")
        moves = [
            move for move in candidates[0].moves
            if crl.norm(move.id) == row["move"]
        ]
        if (
            len(moves) != 1
            or int(moves[0].pp) != row["current_pp"]
            or bool(moves[0].disabled) is not row["world_disabled"]
        ):
            raise Cycle27Error("search engine lost world disabled/legal state")
...
def _process_root(
    ...
    try:
        if c13.sha256(Path(selected["raw_path"])) != selected["raw_sha256"]:
            raise Cycle27Error("selected raw replay hash changed")
        subprocess.run([
        ...
        if not target["actionable"] or target["pp_disable_sidecar"].get("revival_prompt"):
            raise Cycle27Error("selected state is nonordinary actionable semantics")
        actions = c13.request_actions_exact(target["private_request"])
        if actions != set(target["legal_actions"]):
            raise Cycle27Error("exact request action extractor disagrees")
        ...
        for schedule_index in range(SCHEDULE_COUNT):
            ...
            for _repeat in range(2):
                ...
                if len(worlds) != WORLD_COUNT:
                    raise Cycle27Error("production sampler returned wrong world count")
                ...
                if before_weights != after_weights:
                    raise Cycle27Error("move verification changed raw sampler weights")
                payloads = []
                receipt_rows = []
                for sampled, _weight in worlds:
                    receipt = getattr(sampled, crl.MOVE_RECEIPT_ATTRIBUTE, None)
                    if (
                        not isinstance(receipt, Mapping)
                        or receipt.get("schema")
                        != "metagross-causal-move-world-receipts/v1"
                        or receipt.get("battle_tag") != ledger.battle_tag
                        or receipt.get("protocol_sha256") != ledger.protocol_sha256
                        or len(receipt.get("moves", ())) != intrinsic_count
                    ):
                        raise Cycle27Error("typed disable receipt coverage disagrees")
                    if any(
                        row.get("disable_authority") not in {
                            "causal_disable", "world_mechanical_disable"
                        }
                        or not isinstance(row.get("world_disabled"), bool)
                        or (
                            row.get("disable_authority") == "causal_disable"
                            and not isinstance(row.get("causal_disabled"), bool)
                        )
                        or (
                            row.get("disable_authority") == "world_mechanical_disable"
                            and row.get("causal_disabled") is not None
                        )
                        or (
                            row.get("causal_disable_lifecycle_state") is not None
                            and not isinstance(
                                row.get("causal_disable_lifecycle_state"), bool
                            )
                        )
                        or (
                            row.get("disable_authority") == "causal_disable"
                            and row.get("causal_disable_lifecycle_state") is not True
                        )
                        for row in receipt["moves"]
                    ):
                        raise Cycle27Error("typed disable receipt authority is invalid")
                    _assert_engine_disable_receipts(sampled, receipt, search_main)
                    payload, elapsed, _public_size = c13._world_payload(
                        sampled, ledger, actions, engine, search_main,
                    )
                    payloads.append(payload)
                    receipt_rows.append(copy.deepcopy(receipt))
                    conversion_ms.append(elapsed)
                repeats.append({
                    "worlds": payloads,
                    "weights_sha256": hashlib.sha256(after_weights).hexdigest(),
                    "disable_receipts_sha256": c13.hash_json(receipt_rows),
                    "causal_disable_receipts": sum(
                        row["disable_authority"] == "causal_disable"
                        for receipt in receipt_rows for row in receipt["moves"]
                    ),
                    "world_mechanical_disable_receipts": sum(
                        row["disable_authority"] == "world_mechanical_disable"
                        for receipt in receipt_rows for row in receipt["moves"]
                    ),
                })
            if repeats[0] != repeats[1]:
                raise Cycle27Error("seeded production schedule repeat disagreed")
            public_hashes = {row["public_sha256"] for row in repeats[0]["worlds"]}
            if len(public_hashes) != 1:
                raise Cycle27Error("hidden completions changed public projection")
            schedule_rows.append({
                ...
                "disable_receipts_sha256": repeats[0]["disable_receipts_sha256"],
                "causal_disable_receipts": repeats[0]["causal_disable_receipts"],
                "world_mechanical_disable_receipts": repeats[0][
                    "world_mechanical_disable_receipts"
                ],
                "world_count": WORLD_COUNT,
            })
        if len({row["public_sha256"] for row in schedule_rows}) != 1:
            raise Cycle27Error("independent schedules changed public projection")
        result = {
            "schema": "metagross-cycle27-disable-authority-root/v1",
            "status": "pass",
            ...
            "causal_disable_state_count": sum(
                state.disable_authority == "causal_disable"
                for fact in ledger.facts for state in fact.move_states
            ),
            "world_mechanical_disable_state_count": sum(
                state.disable_authority == "world_mechanical_disable"
                for fact in ledger.facts for state in fact.move_states
            ),
        ...
        if os.environ.get("METAGROSS_CYCLE27_PREFREEZE_DEBUG") == "1":
...
def worker(run_dir: Path, index: int) -> None:
    ...
    if not all(hasattr(poke_engine, name) for name in (
        ...
        raise Cycle27Error("request-authoritative mask/ability engine ABI is absent")
...
def parent(run_dir: Path) -> None:
    ...
    if (
        ...
        raise Cycle27Error("frozen 200-root TRAIN selection changed")
    output = run_dir / "mechanics-audit"
    if output.exists():
        raise Cycle27Error("Cycle27 measurement output already exists")
    ...
    for index in range(200):
        ...
        if completed.returncode != 0:
            raise Cycle27Error(f"isolated worker crashed before fail-closed row:{index}")
        if (index + 1) % 20 == 0:
    ...
    gates = {
        ...
        "typed_disable_authority_for_every_admitted_move": all(
            row["intrinsic_revealed_move_count"]
            == row["causal_disable_state_count"]
            + row["world_mechanical_disable_state_count"]
            for row in passed
        ),
    ...
    report = {
        "schema": "metagross-cycle27-disable-authority-report/v1",
