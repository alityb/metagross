#!/usr/bin/env python3
"""Cycle29 TRAIN-only production sampler noninterference audit."""
...
class Cycle29Error(RuntimeError):
    pass


def mechanical_bytes(world: Any) -> bytes:
    return canonical_mechanical_projection(world)
...
def fixed_search_action(
    ...
    if total_weight <= 0:
        raise Cycle29Error("fixture weights have no mass")
    receipts = []
    for index, (state_string, weight) in enumerate(zip(states, weights, strict=True)):
        ...
        if set(by_action) != set(actions) or int(result.total_visits) != MCTS_ITERATIONS:
            raise Cycle29Error("fixed parity search changed action/visit support")
...
def process_fixture(
    ...
    try:
        if c13.sha256(Path(selected["raw_path"])) != selected["raw_sha256"]:
            raise Cycle29Error("selected replay hash changed")
        ...
        if completed.returncode != 0:
            raise Cycle29Error("causal replay capture failed")
        ...
        if set(actions) != set(target["legal_actions"]):
            raise Cycle29Error("request action contract changed")
        ...
        if (
            ...
            raise Cycle29Error("fixture causal class changed")
        ...
        for count in COUNTS:
            ...
            if any(len(rows) != count for rows in (plain, hooked, repeat)):
                raise Cycle29Error("adaptive sampler count changed")
            weight_sets = [
            ...
            if weight_sets[0] != weight_sets[1] or weight_sets[1] != weight_sets[2]:
                raise Cycle29Error("adaptive sampler weights changed")
            mechanical = [
            ...
            if mechanical[0] != mechanical[1] or mechanical[1] != mechanical[2]:
                raise Cycle29Error("receipt hook changed sampled mechanical bytes")
            state_sets = [
                engine_states(rows, ledger, engine, search_main)
                for rows in (plain, hooked, repeat)
            ]
            if state_sets[0] != state_sets[1] or state_sets[1] != state_sets[2]:
                raise Cycle29Error("receipt hook changed engine bytes")
            ...
            if any(
                ...
                raise Cycle29Error("typed production receipt coverage changed")
            if selected["cycle28_fixture_class"] == "opening_empty" and any(
                ...
                raise Cycle29Error("opening empty receipt gained facts")
            ...
            if plain_action != hook_action or plain_search != hook_search:
                raise Cycle29Error("fixed search/selector action changed")
            schedules.append({
        ...
        shutil.rmtree(temp)
        return {
            "schema": "metagross-cycle29-canonical-projection-fixture/v1",
            "status": "pass",
...
def parent(run_dir: Path) -> None:
    ...
    if (
        ...
        raise Cycle29Error("frozen Cycle29 TRAIN fixtures changed")
    output = run_dir / "mechanics-audit"
    if output.exists():
        raise Cycle29Error("Cycle29 measurement output already exists")
    ...
    for index in range(16):
        ...
        if completed.returncode != 0:
            raise Cycle29Error(f"Cycle29 isolated worker crashed:{index}")
    ...
    report = {
        "schema": "metagross-cycle29-canonical-projection-report/v1",
