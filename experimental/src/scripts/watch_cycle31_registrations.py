#!/usr/bin/env python3
"""Cycle31 registration witness for the fresh candidate-attributed smoke."""
...
def validate_registration(payload: dict, pair: dict, username: str) -> str:
    if not re.fullmatch(r"c31smk[xy][0-9a-f]{7}", username):
        raise RuntimeError("unexpected Cycle31 registration username")
    shared = {
    ...
    if any(payload.get(key) != value for key, value in shared.items()):
        raise RuntimeError("Cycle31 registration identity differs from frozen pair")
    ...
    if assigned == pair["team_1_sha256"]:
        ...
        raise RuntimeError("Cycle31 registration has unknown assigned team")
    packed = payload.get("packed_team")
    if not isinstance(packed, str) or packed != expected_team or sha_bytes(packed.encode()) != assigned:
        raise RuntimeError("Cycle31 packed team differs from frozen pair")
    return orientation
...
def watch(directory: Path, pair_manifest: Path, timeout_seconds: float) -> dict:
    ...
    if not isinstance(pairs, list) or len(pairs) != 1:
        raise RuntimeError("Cycle31 watcher requires one frozen pair")
    pair = pairs[0]
    seen: dict[str, dict] = {}
    deleted: set[str] = set()
    empty_since = None
    ...
    while time.monotonic() < deadline:
        ...
        if len(current) > 2:
            raise RuntimeError("Cycle31 registration directory contains extras")
        for username, path in current.items():
            if username in deleted:
                raise RuntimeError("Cycle31 registration reappeared")
            try:
                raw = path.read_bytes()
            ...
            if username in seen and seen[username] != record:
                raise RuntimeError("Cycle31 registration mutated")
        ...
        if len(seen) == 2 and len(deleted) == 2 and not current:
            ...
            if time.monotonic() - empty_since >= 1.0:
                if {row["orientation"] for row in seen.values()} != {"p1", "p2"}:
                    raise RuntimeError("Cycle31 registrations do not cover p1/p2")
                return {
                    "schema": "metagross-cycle31-registration-consumption/v1",
            ...
            empty_since = None
        time.sleep(0.01)
    raise TimeoutError("Cycle31 registration consumption was not observed exactly")
