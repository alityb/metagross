#!/usr/bin/env python3
"""Fail-closed monitor for Cycle29's fresh second-decision live smoke."""
    ...
    for row in protocol:
        ...
        for line in str(row.get("message", "")).splitlines():
            ...
            if (
                ...
                return parts[2]
    raise RuntimeError("Cycle29 cannot identify candidate role")
...
def selected_action_window(
    ...
    if not requests or not sends:
        raise RuntimeError("Cycle29 request/send boundary is incomplete")
    request_ns, request = requests[-1]
    send_ns, messages = sends[0]
    rqid = str(request.get("rqid"))
    if not request_ns < decision_ns < send_ns:
        raise RuntimeError("Cycle29 decision lies outside request/send window")
    if len(messages) != 2 or str(messages[1]) != rqid:
        raise RuntimeError("Cycle29 command rqid differs from exact request")
    ...
    if execution_ns is None:
        raise RuntimeError("Cycle29 selected action was not publicly executed")
    return {
        "rqid": rqid,
...
def opponent_move_before_second_root(
    ...
    if not found:
        raise RuntimeError("Cycle29 second root has no preceding opponent move")
    return found[-1]


def validate_move_receipts(
    run: Path, identity: dict, boundary: dict, *, require_revealed_move: bool
) -> dict:
    files = sorted((run / "move-receipts").glob("agenta-*.jsonl"))
    if len(files) != 1:
        raise RuntimeError("Cycle29 requires one candidate move-receipt file")
    ...
    for row in rows(files[0]):
        ...
        if row.get("schema") != "metagross-causal-move-conversion-receipt/v1":
            raise RuntimeError("Cycle29 move receipt schema changed")
        if row.get("observer_role") != "p1" or row.get("swap") is not False:
            raise RuntimeError("Cycle29 move receipt has wrong perspective")
        if int(row.get("receipt_time_ns", 0)) > boundary_ns:
            raise RuntimeError("Cycle29 move receipt occurred after public execution")
        ...
        if (
            ...
            or not isinstance(move_receipt.get("derived_executions"), list)
        ):
            raise RuntimeError("Cycle29 nested move receipt identity changed")
        if require_revealed_move and not move_receipt["moves"]:
            raise RuntimeError("Cycle29 second root did not carry a revealed move")
        if not require_revealed_move and (
            move_receipt["moves"] or move_receipt["derived_executions"]
        ):
            raise RuntimeError("Cycle29 opening root receipt is not empty")
        for event in move_receipt["derived_executions"]:
            if (
                not isinstance(event, dict)
                or event.get("authority") != "derived_public_execution"
                or not isinstance(event.get("derived_cause"), str)
                or not event["derived_cause"]
            ):
                raise RuntimeError("Cycle29 derived move provenance is invalid")
            derived_executions += 1
        for move in move_receipt["moves"]:
            authority = move.get("disable_authority")
            if authority not in authority_counts:
                raise RuntimeError("Cycle29 move has unknown disable authority")
            if (
                ...
                raise RuntimeError("Cycle29 move PP/disable receipt is malformed")
            if authority == "causal_disable" and move.get("world_disabled") is not True:
                raise RuntimeError("Cycle29 causal disable did not reach search state")
        ...
        if phase == "production_control" and cohort == "adaptive_root_search":
            ...
            raise RuntimeError("Cycle29 move receipt has unknown execution cohort")
    if len(protocol_hashes) != 1:
        raise RuntimeError("Cycle29 second-root receipts span causal prefixes")
    declared = {row.get("declared_world_count") for row in production}
    if len(declared) != 1 or next(iter(declared)) not in {16, 32}:
        raise RuntimeError("Cycle29 production world declaration is invalid")
    ...
    if (
        ...
        raise RuntimeError("Cycle29 production move receipts do not reconcile")
    ...
    if (
        ...
        raise RuntimeError("Cycle29 candidate move receipts are not exact 2x8")
    return {
        ...
        "derived_execution_rows": derived_executions,
        "opening_empty_receipt": not require_revealed_move,
...
def engine_provenance(run: Path, expected_engine_sha: str) -> list[dict] | None:
    ...
    for namespace in ("agent_a", "agent_b"):
        ...
        if (
            ...
            raise RuntimeError("Cycle29 spawned process engine provenance changed")
...
def try_validate(run: Path, pair_manifest: Path, expected_engine_sha: str) -> dict | None:
    ...
    if len(candidate_files) != 1:
        raise RuntimeError("Cycle29 candidate identity is not unique")
    search_path, candidates = candidate_files[0]
    first = [
        row
        for row in candidates
        if (row.get("context") or {}).get("decision_idx") == 0
    ]
    second = [
        row
        for row in candidates
    ...
    first_row = first[0]
    first_teacher = (
        (first_row.get("choice_override") or {}).get("terminal_mcts_teacher") or {}
    )
    validate_teacher(first_teacher)
    first_selected = str(first_teacher["selected_action"])
    first_override = first_row.get("choice_override") or {}
    if (
        first_row.get("choice") != first_selected
        or first_override.get("final_choice") != first_selected
        or str(first_teacher.get("reason", "")).startswith("fail_closed")
    ):
        raise RuntimeError("Cycle29 opening candidate action was not installed")
    row = second[0]
    ...
    if row.get("choice") != selected or override.get("final_choice") != selected:
        raise RuntimeError("Cycle29 candidate selected action was not installed")
    if str(teacher.get("reason", "")).startswith("fail_closed"):
        raise RuntimeError("Cycle29 candidate fell back")
    ...
    if (
        ...
        raise RuntimeError("Cycle29 public boundary identity differs from second root")
    protocol_path = search_path.with_name(
        search_path.name.removesuffix(".search.jsonl") + ".protocol.jsonl"
    )
    protocol = rows(protocol_path)
    first_window = selected_action_window(
        protocol,
        int(first_row["time_ns"]),
        search_path.name.removesuffix(".search.jsonl"),
        first_selected,
    )
    ...
    if int(boundary["public_execution_time_ns"]) < window["public_execution_time_ns"]:
        raise RuntimeError("Cycle29 latch predates public execution")
    preceding = opponent_move_before_second_root(
        protocol, role=window["observer_role"], before_ns=int(row["time_ns"])
    )
    first_context = first_row.get("context") or {}
    first_identity = {
        "battle_tag": first_context.get("tag"),
        "rqid": first_context.get("rqid"),
        "decision_index": first_context.get("decision_idx"),
        "root_id": hashlib.sha256(
            f"terminal-mcts-live\0{first_context.get('tag')}\0{first_context.get('decision_idx')}".encode()
        ).hexdigest(),
    }
    opening_receipts = validate_move_receipts(
        run, first_identity, boundary, require_revealed_move=False
    )
    move_receipts = validate_move_receipts(
        run, identity, boundary, require_revealed_move=True
    )
    if not rows(run / "prior-a.jsonl"):
        return None
    return {
        "schema": "metagross-cycle29-second-root-operational-smoke/v1",
        "status": "pass",
        ...
        "decision_index": 1,
        "opening_decision": {
            "selected_action": first_selected,
            "request_decision_execution_window": first_window,
            "typed_move_receipts": opening_receipts,
        },
        "worlds": 16,
...
def main() -> None:
    ...
    while time.monotonic() < deadline:
        ...
        if result is not None:
            ...
            return
        time.sleep(0.01)
    raise TimeoutError("Cycle29 second-root smoke did not pass")
