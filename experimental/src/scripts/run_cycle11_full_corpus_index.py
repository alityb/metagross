#!/usr/bin/env python3
"""Run Cycle 11's frozen exact-format full-corpus coverage/index gate."""
...
SPLIT_DOMAIN = "cycle11-split-20260815"
SCHEMA = "metagross-cycle11-compact-battle-index/v1"
...
def main() -> None:
    ...
    if args.workers != 8:
        raise ValueError("frozen Cycle 11 worker count is exactly eight")
    _install_contract()
    ...
    if len(corpus) != 20564 or len(positives) != 20560 or len(negatives) != 4:
        raise ValueError("frozen Cycle 11 corpus cardinality changed")
    if len(spot) != 256 or len(exclusions) != 69:
        raise ValueError("frozen Cycle 11 spot/exclusion cardinality changed")
    ...
    report = {
        "schema": "metagross-cycle11-full-corpus-report/v1", "status": status,
