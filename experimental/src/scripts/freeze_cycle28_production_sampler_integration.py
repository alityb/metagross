#!/usr/bin/env python3
"""Freeze Cycle28 TRAIN fixtures, implementation, runtimes, and gates."""
...
RUN = ROOT / "experimental/runs/search_native_v2_cycle28_production_sampler_integration_20260815"
...
def main() -> None:
    if MANIFEST.exists():
        raise SystemExit("Cycle28 manifest already exists")
    tests = json.loads((RUN / "PREFREEZE_TESTS.json").read_text())
    if tests.get("status") != "pass" or tests.get("passed") != 16:
        raise SystemExit("Cycle28 tests did not pass")
    ...
    if (
        ...
        raise SystemExit("Cycle28 fixture selection violates frozen contract")
    ...
    sources = [
        ...
        ROOT / "experimental/src/scripts/select_cycle28_train_fixtures.py",
        ROOT / "experimental/src/scripts/audit_cycle28_production_sampler_integration.py",
        ROOT / "experimental/src/scripts/freeze_cycle28_production_sampler_integration.py",
    ...
    if any(not path.is_file() for path in sources):
        raise SystemExit("Cycle28 frozen source is missing")
    ...
    for commit in commits:
        ...
        if actual != commit:
            raise SystemExit("Cycle28 Showdown worktree commit mismatch")
        runtimes.append({
            "commit": commit,
            "path": str(worktree),
    ...
    payload = {
        "schema": "metagross-cycle28-production-sampler-manifest/v1",
