#!/usr/bin/env python3
"""Derive Cycle31 prepare/live smoke argv from one canonical object."""
...
def load_canonical(path: Path) -> tuple[Path, list[str]]:
    ...
    if payload.get("schema") != "metagross-cycle31-canonical-smoke-argv/v1":
        raise RuntimeError("wrong Cycle31 canonical argv schema")
    evaluator = Path(payload["evaluator"]).resolve()
    argv = payload.get("argv")
    if evaluator != (ROOT / "experimental/src/eval/run.py").resolve():
        raise RuntimeError("Cycle31 evaluator path changed")
    if not isinstance(argv, list) or not argv or any(not isinstance(x, str) for x in argv):
        raise RuntimeError("Cycle31 canonical argv is malformed")
    if {"--prepare-mirrored-pairs-only", "--pair-manifest-sha256"}.intersection(argv):
        raise RuntimeError("Cycle31 canonical argv contains projected fields")
...
def derived_argv(canonical: Path, phase: str, pair_sha256: str | None = None):
    ...
    if phase == "prepare":
        ...
        if pair_sha256 is None or len(pair_sha256) != 64:
            raise RuntimeError("Cycle31 live phase requires frozen pair SHA")
        derived += ["--pair-manifest-sha256", pair_sha256]
    else:
        raise RuntimeError("unknown Cycle31 phase")
...
def main() -> None:
    ...
    if args.phase == "verify":
        if args.pair_manifest is None or args.pair_sha256 is None:
            raise RuntimeError("Cycle31 verify requires pair manifest and SHA")
        if sha(args.pair_manifest) != args.pair_sha256:
            raise RuntimeError("Cycle31 pair bytes differ from frozen SHA")
        ...
        if prepared != live or prepared != stored:
            raise RuntimeError("Cycle31 preparation/live identity mismatch")
