...
def validate_registration(payload: dict, pair: dict, username: str) -> str:
    if not re.fullmatch(r"c21smk[xy][0-9a-f]{7}", username):
        raise RuntimeError("unexpected Cycle21 registration username")
    shared = {
        "schema_version": 1,
    ...
    if any(payload.get(key) != value for key, value in shared.items()):
        raise RuntimeError("Cycle21 registration identity differs from frozen pair")
    ...
    if assigned == pair["team_1_sha256"]:
        ...
        raise RuntimeError("Cycle21 registration has unknown assigned team")
    packed = payload.get("packed_team")
    if not isinstance(packed, str) or sha_bytes(packed.encode()) != assigned or packed != expected_team:
        raise RuntimeError("Cycle21 registration packed team does not match manifest")
    return orientation
...
def watch(directory: Path, pair_manifest: Path, timeout_seconds: float) -> dict:
    ...
    if not isinstance(pairs, list) or len(pairs) != 1:
        raise RuntimeError("Cycle21 watcher requires one frozen pair")
    pair = pairs[0]
    seen: dict[str, dict] = {}
    deleted: set[str] = set()
    empty_since = None
    ...
    while time.monotonic() < deadline:
        ...
        if len(current_paths) > 2:
            raise RuntimeError("Cycle21 pair directory contains extra registrations")
        for username, path in current_paths.items():
            if username in deleted:
                raise RuntimeError("Cycle21 registration reappeared after consumption")
            try:
                raw = path.read_bytes()
            ...
            if previous is not None and previous != record:
                raise RuntimeError("Cycle21 registration mutated before consumption")
        ...
        if len(seen) == 2 and len(deleted) == 2 and not current_paths:
            ...
            if time.monotonic() - empty_since >= 1.0:
                ...
                if orientations != {"p1", "p2"}:
                    raise RuntimeError("Cycle21 registrations do not cover p1/p2 teams")
                return {
                    "schema": "metagross-cycle21-registration-consumption/v1",
            ...
            empty_since = None
        time.sleep(0.01)
    raise TimeoutError("Cycle21 registration consumption was not observed exactly")
