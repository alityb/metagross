...
if [[ "${METAGROSS_CYCLE31_EXECUTE:-}" != "FROZEN_CANDIDATE_ATTRIBUTED_BOUNDARY_SMOKE" ]]; then
  echo "set METAGROSS_CYCLE31_EXECUTE=FROZEN_CANDIDATE_ATTRIBUTED_BOUNDARY_SMOKE" >&2; exit 2
fi
PYTHONPATH="$ROOT/experimental/src:$ROOT" "$ROOT/.venv-metamon/bin/python" \
  "$ROOT/experimental/src/scripts/verify_cycle31_presmoke.py" "$MANIFEST" >/dev/null
PYTHONPATH="$ROOT/experimental/src:$ROOT" "$ROOT/.venv-metamon/bin/python" \
  "$ROOT/experimental/src/scripts/cycle31_canonical_smoke.py" --canonical "$CANONICAL" \
...
[[ ! -e "$RUN/SMOKE_RESULT.json" && ! -e "$RUN/REGISTRATION_CONSUMPTION.json" \
  ...
  echo "Cycle31 live outputs already exist" >&2; exit 2;
}
...
cleanup() {
  ...
  while read -r pid; do [[ -n "$pid" ]] && kill "$pid" 2>/dev/null || true; done \
    < <(pgrep -f 'srcs.metagross.run_foul_play.*c31smk' || true)
}
trap cleanup EXIT INT TERM

PYTHONPATH="$ROOT/experimental/src:$ROOT" "$ROOT/.venv-metamon/bin/python" -u \
  "$ROOT/srcs/metagross/showdown_runtime_server.py" \
  --runtime-manifest "$RUNTIME_MANIFEST" --pair-directory "$RUN/smoke-registrations" \
...
for side in a b; do
  ...
  env METAMON_CACHE_DIR="$ROOT/external/metamon_cache" TORCHDYNAMO_DISABLE=1 ACCELERATE_USE_CPU=true CUDA_VISIBLE_DEVICES='' \
    ...
    --username "c31smk${side}" --port "$port" --decision-dump "$RUN/prior-${side}.jsonl" \
...
env METAGROSS_CYCLE31_BOUNDARY_RECEIPT="$RUN/PUBLIC_EXECUTION_BOUNDARY.json" \
  METAGROSS_CYCLE31_MAX_DECISION_INDEX=5 METAGROSS_CYCLE31_MAX_BATTLE_TURN=6 \
...
PYTHONPATH="$ENGINE_ROOT:$ROOT/srcs/vendor/foul-play:$ROOT/experimental/src:$ROOT" \
  "$ROOT/.venv-metamon/bin/python" "$ROOT/experimental/src/scripts/monitor_cycle31_candidate_boundary_smoke.py" \
