#!/usr/bin/env python3
"""Freeze Cycle 38c crowned-switch temporal lineage repair."""
...
from experimental.src.scripts.audit_cycle38_temporal_request_lineage import (
    ORICORIO_PP_ROOT, TEMPORAL_ROOTS, selected_rows,
)
from experimental.src.scripts.run_cycle8_replay_audit import tree_sha256, verify_manifest


ROOT = Path(__file__).resolve().parents[3]
RUN = ROOT / "experimental/runs/search_native_v2_cycle38c_temporal_switch_20260816"
...
def main() -> None:
    if MANIFEST.exists():
        raise SystemExit("Cycle38c manifest exists")
    ...
    if (
        ...
        raise SystemExit("Cycle38c fixed TRAIN selection changed")
    pretests = json.loads((RUN / "PREFREEZE_TESTS.json").read_text())
    if pretests.get("total_passed") != 80 or pretests.get("failed") != 0:
        raise SystemExit("Cycle38c prefreeze tests failed")
    ...
    paths = [
        ...
        ROOT / "experimental/src/scripts/freeze_cycle38c_temporal_switch.py",
        ...
        ROOT / "experimental/runs/search_native_v2_cycle38b_temporal_request_20260816/mechanics-audit/REPORT.json",
        ENGINE, Path(shutil.which("node") or ""),
    ]
    paths.extend(Path(row["raw_path"]) for row in selected)
    if any(not path.is_file() for path in paths):
        raise SystemExit("Cycle38c frozen source missing")
    runtimes = []
    for commit in sorted({row["showdown_commit"] for row in selected}):
        ...
        if actual != commit:
            raise SystemExit("Cycle38c Showdown runtime mismatch")
        runtimes.append({
            "commit": commit, "path": str(worktree),
            "dist_tree_sha256": tree_sha256(worktree / "dist"),
        })
    payload = {
        "schema": "metagross-cycle38c-temporal-switch-freeze/v1",
        "protocol_sha256": sha256(RUN / "PROTOCOL.md"),
        "cycle38b_report_sha256": sha256(ROOT / (
            "experimental/runs/search_native_v2_cycle38b_temporal_request_20260816/"
            "mechanics-audit/REPORT.json"
        )),
        ...
        "root_count": 42, "expected_supported_root_count": 41,
        "expected_supported_world_count": 656, "split": "train",
        "temporal_failure_fingerprints": sorted(TEMPORAL_ROOTS),
        "expected_oricorio_pp_failure_fingerprint": ORICORIO_PP_ROOT,
        "prefreeze_tests_passed": True, "prefreeze_test_count": 80,
        "files": [
        ...
        "gates": {
            "temporal_roots_pass": 10, "non_pp_roots_pass": 41,
            "supported_worlds": 656, "expected_pp_control_failures": 1,
            "unexpected_failures_max": 0, "move_verification_p95_ms_max": 5.0,
            "isolated_root_p95_ms_max": 1750.0,
        },
        "authorization": {
            "cycle39_pp_gate_after_pass": True, "fresh_h2h": False,
