...
PAIR_SHA="e5e9b1781e04ec76a9da056e1d4b3070935e60da685dc1cbd73d2d9cc16dd6b6"
...
if [[ "${METAGROSS_CYCLE24_EXECUTE:-}" != "FROZEN_CANONICAL_FIRST_DECISION_SMOKE" ]]; then
  echo "set METAGROSS_CYCLE24_EXECUTE=FROZEN_CANONICAL_FIRST_DECISION_SMOKE" >&2; exit 2
fi
PYTHONPATH="$ROOT/experimental/src:$ROOT" "$ROOT/.venv-metamon/bin/python" \
  "$ROOT/experimental/src/scripts/verify_cycle24_presmoke.py" "$MANIFEST" >/dev/null
PYTHONPATH="$ROOT/experimental/src:$ROOT" "$ROOT/.venv-metamon/bin/python" \
  "$ROOT/experimental/src/scripts/cycle24_canonical_eval.py" --canonical "$CANONICAL" \
...
[[ ! -e "$RUN/SMOKE_RESULT.json" && ! -e "$RUN/REGISTRATION_CONSUMPTION.json" \
  ...
  echo "Cycle24 live outputs already exist" >&2; exit 2;
}
...
cleanup() {
  ...
  while read -r pid; do [[ -n "$pid" ]] && kill "$pid" 2>/dev/null || true; done \
    < <(pgrep -f 'srcs.metagross.run_foul_play.*c24smk' || true)
}
...
for side in a b; do
  ...
  env METAMON_CACHE_DIR="$ROOT/external/metamon_cache" TORCHDYNAMO_DISABLE=1 ACCELERATE_USE_CPU=true CUDA_VISIBLE_DEVICES='' \
    ...
    --username "c24smk${side}" --port "$port" --decision-dump "$RUN/prior-${side}.jsonl" \
