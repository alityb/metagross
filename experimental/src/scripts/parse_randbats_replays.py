...
def _parse_one(task: tuple[str, str]) -> tuple[str, str]:
    gameid, path = task
    global _worker_parser
    try:
        ...
        return (gameid, f"{type(e).__name__}: {str(e)[:100]}")
...
def parse_replay_dir(
    ...
    unexpected = set(outputs_by_gameid) - set(raw_by_gameid)
    if unexpected:
        raise ValueError(f"parsed POVs without raw replay: {sorted(unexpected)}")

    todo = [(gameid, path) for gameid, path in raw_by_gameid.items() if gameid not in outputs_by_gameid]
    ok = 0
    failed = 0
    if workers == 1:
        _init_worker(str(out_dir), str(pool_path), seed)
        results = list(_parse_one((gameid, str(path))) for gameid, path in todo)
    elif todo:
        with mp.Pool(
            workers,
            ...
            results = list(pool.imap_unordered(_parse_one, ((gameid, str(path)) for gameid, path in todo)))
    else:
        results = []

    for gameid, status in results:
        pov_paths = list(out_dir.glob(f"{gameid}_*.json.lz4"))
        if status == "ok" and len(pov_paths) == 2:
            ok += 1
            continue
        # Metamon handles known parser failures by warning and returning. Do not
        # count that as success, and leave no partial result to be reused later.
