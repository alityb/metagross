#!/usr/bin/env python3
"""Verify every frozen Cycle31 smoke input."""
...
def verify(path: Path) -> dict:
    ...
    if payload.get("schema") != "metagross-cycle31-candidate-attributed-presmoke-freeze/v1":
        raise RuntimeError("wrong Cycle31 presmoke manifest schema")
    for row in payload["files"]:
        source = Path(row["path"])
        if not source.is_file() or sha(source) != row["sha256"]:
            raise RuntimeError(f"Cycle31 frozen file mismatch: {source}")
    ...
    if commit != showdown["commit"] or tree_sha256(root / "dist") != showdown["dist_tree_sha256"]:
        raise RuntimeError("Cycle31 Showdown runtime changed")
    return payload
