...
PAIR_SHA="e55a985c1162faf5aac96ea87b7264427ac146549c891df14ae79040bd233bfc"
...
if [[ "${METAGROSS_CYCLE22_EXECUTE:-}" != "FROZEN_CERTIFIED_ABILITY_SMOKE" ]]; then
  echo "set METAGROSS_CYCLE22_EXECUTE=FROZEN_CERTIFIED_ABILITY_SMOKE" >&2; exit 2
fi
PYTHONPATH="$ROOT/experimental/src:$ROOT" "$ROOT/.venv-metamon/bin/python" \
  "$ROOT/experimental/src/scripts/verify_cycle22_presmoke.py" "$MANIFEST" >/dev/null
...
[[ ! -e "$RUN/SMOKE_RESULT.json" && ! -e "$RUN/REGISTRATION_CONSUMPTION.json" \
  ...
  echo "Cycle22 live outputs already exist" >&2; exit 2
}
...
cleanup() {
  ...
  while read -r pid; do [[ -n "$pid" ]] && kill "$pid" 2>/dev/null || true; done \
    < <(pgrep -f 'srcs.metagross.run_foul_play.*c22smk' || true)
}
...
for side in a b; do
  ...
  env METAMON_CACHE_DIR="$ROOT/external/metamon_cache" TORCHDYNAMO_DISABLE=1 ACCELERATE_USE_CPU=true CUDA_VISIBLE_DEVICES='' \
    ...
    --username "c22smk${side}" --port "$port" --decision-dump "$RUN/prior-${side}.jsonl" \
...
env METAMON_CACHE_DIR="$ROOT/external/metamon_cache" TORCHDYNAMO_DISABLE=1 ACCELERATE_USE_CPU=true CUDA_VISIBLE_DEVICES='' \
  ...
  --websocket-uri ws://127.0.0.1:8010/showdown/websocket --paired --mirrored-pairs --mirror-seed 202622001317 \
  ...
  --production-run-seed 2222222222222222222222222222222222222222222222222222222222222222 \
  --concurrent-games 1 --fail-fast --game-timeout-seconds 900 --n-games 2 \
  --username-prefix c22smk --run-id cycle22-certified-ability-smoke --pair-manifest-sha256 "$PAIR_SHA" \
...
PYTHONPATH="$ENGINE_ROOT:$ROOT/srcs/vendor/foul-play:$ROOT/experimental/src:$ROOT" \
  "$ROOT/.venv-metamon/bin/python" "$ROOT/experimental/src/scripts/monitor_cycle22_certified_ability_smoke.py" \
