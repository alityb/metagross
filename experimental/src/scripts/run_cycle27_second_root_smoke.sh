...
if [[ "${METAGROSS_CYCLE27_EXECUTE:-}" != "FROZEN_TYPED_SECOND_ROOT_SMOKE" ]]; then
  echo "set METAGROSS_CYCLE27_EXECUTE=FROZEN_TYPED_SECOND_ROOT_SMOKE" >&2; exit 2
fi
PYTHONPATH="$ROOT/experimental/src:$ROOT" "$ROOT/.venv-metamon/bin/python" \
  "$ROOT/experimental/src/scripts/verify_cycle27_presmoke.py" "$MANIFEST" >/dev/null
PYTHONPATH="$ROOT/experimental/src:$ROOT" "$ROOT/.venv-metamon/bin/python" \
  "$ROOT/experimental/src/scripts/cycle27_canonical_smoke.py" --canonical "$CANONICAL" \
...
[[ ! -e "$RUN/SMOKE_RESULT.json" && ! -e "$RUN/REGISTRATION_CONSUMPTION.json" \
  ...
  echo "Cycle27 live outputs already exist" >&2; exit 2;
}
...
cleanup() {
  ...
  while read -r pid; do [[ -n "$pid" ]] && kill "$pid" 2>/dev/null || true; done \
    < <(pgrep -f 'srcs.metagross.run_foul_play.*c27smk' || true)
}
...
for side in a b; do
  ...
  env METAMON_CACHE_DIR="$ROOT/external/metamon_cache" TORCHDYNAMO_DISABLE=1 ACCELERATE_USE_CPU=true CUDA_VISIBLE_DEVICES='' \
    ...
    --username "c27smk${side}" --port "$port" --decision-dump "$RUN/prior-${side}.jsonl" \
...
PYTHONPATH="$ENGINE_ROOT:$ROOT/srcs/vendor/foul-play:$ROOT/experimental/src:$ROOT" \
  "$ROOT/.venv-metamon/bin/python" "$ROOT/experimental/src/scripts/monitor_cycle27_second_root_smoke.py" \
