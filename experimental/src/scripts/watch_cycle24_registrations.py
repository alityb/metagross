#!/usr/bin/env python3
"""Cycle24 registration witness with fresh identity domain."""
...
def validate_registration(payload: dict, pair: dict, username: str) -> str:
    if not re.fullmatch(r"c24smk[xy][0-9a-f]{7}", username):
        raise RuntimeError("unexpected Cycle24 registration username")
    shared = {
    ...
    if any(payload.get(key) != value for key, value in shared.items()):
        raise RuntimeError("Cycle24 registration identity differs from frozen pair")
    ...
    if assigned == pair["team_1_sha256"]:
        ...
        raise RuntimeError("Cycle24 registration has unknown assigned team")
    packed = payload.get("packed_team")
    if not isinstance(packed, str) or packed != expected_team or sha_bytes(packed.encode()) != assigned:
        raise RuntimeError("Cycle24 registration packed team does not match manifest")
    return orientation
...
def watch(directory: Path, pair_manifest: Path, timeout_seconds: float) -> dict:
    ...
    if not isinstance(pairs, list) or len(pairs) != 1:
        raise RuntimeError("Cycle24 watcher requires one frozen pair")
    pair = pairs[0]
    seen: dict[str, dict] = {}
    deleted: set[str] = set()
    empty_since = None
    ...
    while time.monotonic() < deadline:
        ...
        if len(current) > 2:
            raise RuntimeError("Cycle24 registration directory contains extras")
        for username, path in current.items():
            if username in deleted:
                raise RuntimeError("Cycle24 registration reappeared")
            try:
                raw = path.read_bytes()
            ...
            record = {
                "username": username, "path": str(path.resolve()), "sha256": sha_bytes(raw),
                "orientation": validate_registration(payload, pair, username), "payload": payload,
            }
            if username in seen and seen[username] != record:
                raise RuntimeError("Cycle24 registration mutated")
        ...
        if len(seen) == 2 and len(deleted) == 2 and not current:
            ...
            if time.monotonic() - empty_since >= 1.0:
                if {row["orientation"] for row in seen.values()} != {"p1", "p2"}:
                    raise RuntimeError("Cycle24 registrations do not cover p1/p2")
                return {
                    "schema": "metagross-cycle24-registration-consumption/v1", "status": "pass",
            ...
            empty_since = None
        time.sleep(0.01)
    raise TimeoutError("Cycle24 registration consumption was not observed exactly")
