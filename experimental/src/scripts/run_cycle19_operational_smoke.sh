...
PAIR_SHA="efbfa7d3df5621374e7e53ccbb8a54b81f35279a288c383b064b7de2810528f5"
...
if [[ "${METAGROSS_CYCLE19_EXECUTE:-}" != "FROZEN_ONE_DECISION_SMOKE" ]]; then
  echo "set METAGROSS_CYCLE19_EXECUTE=FROZEN_ONE_DECISION_SMOKE" >&2
  exit 2
fi
PYTHONPATH="$ROOT" "$ROOT/.venv-metamon/bin/python" \
  "$ROOT/experimental/src/scripts/verify_cycle19_presmoke.py" "$MANIFEST" >/dev/null
[[ "$(shasum -a 256 "$PAIR" | awk '{print $1}')" == "$PAIR_SHA" ]] || exit 2
[[ ! -e "$RUN/SMOKE_RESULT.json" && ! -e "$RUN/prior-a.jsonl" && ! -e "$RUN/prior-b.jsonl" ]] || {
  echo "Cycle19 smoke outputs already exist" >&2
  exit 2
}
...
cleanup() {
  ...
  done < <(pgrep -f 'srcs.metagross.run_foul_play.*c19smk' || true)
}
...
for side in a b; do
  ...
  env METAMON_CACHE_DIR="$ROOT/external/metamon_cache" TORCHDYNAMO_DISABLE=1 \
    ...
    --trajectory-mode causal-history --username "c19smk${side}" --port "$port" \
...
env METAMON_CACHE_DIR="$ROOT/external/metamon_cache" TORCHDYNAMO_DISABLE=1 \
  ...
  --paired --mirrored-pairs --mirror-seed 202619081501 \
  ...
  --production-run-seed 1919191919191919191919191919191919191919191919191919191919191919 \
  --concurrent-games 1 --fail-fast --game-timeout-seconds 900 --n-games 2 \
  --username-prefix c19smk --run-id cycle19-operational-smoke \
...
# The smoke identity is intentionally retired immediately after the first
# publicly confirmed candidate action; no incidental game outcome is evidence.
