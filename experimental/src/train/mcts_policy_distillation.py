...
def _visit_distribution_to_target(
    visits: dict[str, object], state: object, *, ignore_zero_mass_actions: bool
) -> list[float]:
    ...
    for action, value in visits.items():
        ...
        if mass == 0 and ignore_zero_mass_actions:
            continue
...
def visit_distribution_to_target(visits: dict[str, object], state: object) -> list[float]:
    """Convert sparse MCTS visits to a normalized 13-way policy target."""
    return _visit_distribution_to_target(visits, state, ignore_zero_mass_actions=False)


def positive_visit_distribution_to_target(visits: dict[str, object], state: object) -> list[float]:
    """Map every positive visit action, retaining validation of every mass value."""
    return _visit_distribution_to_target(visits, state, ignore_zero_mass_actions=True)
...
def _well_formed_capture_target(record: dict[str, Any]) -> bool:
    """Require a complete numeric capture target before accepting a string remap."""
    selected = record.get("canonical_selected_action_index")
    target = record.get("mcts_visit_target_13")
    if not isinstance(selected, int) or not 0 <= selected < NUM_ACTIONS:
        return False
    if not isinstance(target, list) or len(target) != NUM_ACTIONS:
        return False
    try:
        ...
        return False
    return all(math.isfinite(value) and value >= 0 for value in values) and sum(values) > 0


def remapped_explicit_policy_target(record: dict[str, Any], state: object, parsed_action: int) -> list[float]:
    """Rebuild a schema-v2 target from raw action strings on the parsed state.

    Schema-v2's captured indices describe Foul Play's live action context, which
    can differ from the parser's Metamon action slots. The fallback is only
    available for a structurally valid capture record. Every positive-mass
    string and the selected string must independently map to the parsed state.
    """
    if not _well_formed_capture_target(record):
        raise ActionMappingError("capture target is malformed")
    selected = foul_play_action_to_index(record.get("selected_action"), state)
    if selected != parsed_action:
        raise ActionMappingError("selected action differs from parsed action")
    return positive_visit_distribution_to_target(record.get("mcts_visits"), state)
...
def build_sidecar(
    ...
    remapped_records, remapped_replays = 0, 0
    rejection_reasons: Counter[str] = Counter()
    rejected_povs: list[dict[str, object]] = []
    with output.open("w", encoding="utf-8") as handle:
        for pov, decisions in sorted(rows_by_pov.items()):
            ...
            if trajectory is None:
                ...
                rejected_povs.append(
                    {"battle_tag": pov[0], "learner_pov": pov[1], "reason": "missing_or_ambiguous_learner_pov"}
                )
                continue
            try:
                ...
                explicit_schema = all(row.get("mcts_schema_version") in (2, 3) for _, _, row in decisions)
                if explicit_schema:
                    if trajectory_index is None:
                        rejected += len(decisions)
                        rejection_reasons["explicit_trajectory_index_required"] += len(decisions)
                        rejected_povs.append(
                            {"battle_tag": pov[0], "learner_pov": pov[1], "reason": "explicit_trajectory_index_required"}
                        )
                        continue
                    ...
                    if sequences != list(range(len(ordered))) or len(ordered) != len(states):
                        ...
                        rejected_povs.append(
                            {"battle_tag": pov[0], "learner_pov": pov[1], "reason": "explicit_sequence_mismatch"}
                        )
                        continue
                    records = []
                    remapped = False
                    for timestep, (source, line_number, row) in enumerate(ordered):
                        try:
                            if row.get("canonical_selected_action_index") != actions[timestep]:
                                raise ActionMappingError("canonical selected action differs from parsed action")
                            target = explicit_policy_target(row, states[timestep])
                        except ActionMappingError:
                            try:
                                target = remapped_explicit_policy_target(row, states[timestep], actions[timestep])
                                remapped = True
                            except ActionMappingError:
                                records = []
                                rejected += len(decisions)
                                rejection_reasons["explicit_illegal_or_unmappable_target"] += len(decisions)
                                rejected_povs.append(
                                    {"battle_tag": pov[0], "learner_pov": pov[1], "reason": "explicit_illegal_or_unmappable_target"}
                                )
                                break
                        record = {
                    ...
                    if records and remapped:
                        remapped_replays += 1
                        remapped_records += len(records)
                    continue
                if len(decisions) != len(states) or len(actions) != len(states):
                    rejected += len(decisions)
                    rejection_reasons["decision_action_count_mismatch"] += len(decisions)
                    rejected_povs.append(
                        {"battle_tag": pov[0], "learner_pov": pov[1], "reason": "decision_action_count_mismatch"}
                    )
                    continue
                records = []
                ...
                if str(exc) == "selected action differs from parsed action":
                    ...
                    reason = "illegal_or_unmappable_action"
                rejection_reasons[reason] += len(decisions)
                rejected_povs.append({"battle_tag": pov[0], "learner_pov": pov[1], "reason": reason})
                continue
    ...
    result["remapped_records"] = remapped_records
    result["remapped_replays"] = remapped_replays
    result.update({f"rejected_{reason}": count for reason, count in sorted(rejection_reasons.items())})
    result["rejected_povs"] = rejected_povs
    return result
