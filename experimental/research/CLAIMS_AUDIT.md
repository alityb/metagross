...
| 3 | Completed-Q harmful under determinization | 10-40 (p = 1.2e-5 one-sided), 37% flips | Exact p computed; mechanism (strategy fusion) argued from telemetry | n=50, one seed, UNMIRRORED (Modal lanes lacked enforcement); unreplicated — a 200-game local replication is queued as optional |
| 4 | Flattening: wins H2H, loses population | 55.5% [48.6,62.2] (n=200); ladder 81.7 vs 89.4; league retrodiction rejects it | Mirror-integrity audited (screens unpaired but unbiased); league retro on verified-mirrored games; stratification SOFTENED after endogenous-matchmaking check (mix-standardization run; key cell Fisher p=0.032) | H2H screen unmirrored (power, not bias); cross-day ladder pools (ratings are the invariant evidence) |
| 5 | Priors: no H2H edge vs own skeleton | 88-112 (44.0% [37.3,50.9]), n=200, verified-mirrored | Powered replication of the league cell; activation/enforcement verified | **Population side (7–10 GXE gap) was folklore — now under direct measurement (vanilla_ladder_20260901, running)** |
...
| 7 | Search overrides policy 48.1% | 3,962 telemetered decisions (peak push, stateless mode) | — | Single serving mode/run; cross-check on causal-mode telemetry pending (archived) |
