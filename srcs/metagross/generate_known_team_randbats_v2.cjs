...
// Known-team corpus generator for the final decision-utility experiment.
// The corpus UID is derived from the master seed and battle index so rows from
// different corpora cannot collide when they are joined, split, or bootstrapped.
...
function corpusUid(master, index) {
  return sha256(`${master}\0${index}`).slice(0, 24);
}
...
async function generateBattle(master, index) {
  ...
  const uid = corpusUid(master, index);
  return {
    schema: 'metagross-known-team-battle/v2',
    corpus_uid: uid,
    battle_id: `known-team-v2-${uid}`,
    battle_index: index,
    format: formatID,
    seeds: {
      master,
...
async function main() {
  ...
  if (!Number.isInteger(count) || count < 1 || !output || !master || !Number.isInteger(start) || start < 0) {
    throw new Error('usage: generate_known_team_randbats_v2.cjs COUNT OUTPUT MASTER_SEED [START_INDEX]');
  }
  const flags = start === 0 ? 'w' : 'a';
  const descriptor = fs.openSync(output, flags, 0o600);
  try {
    for (let offset = 0; offset < count; offset++) {
      const battle = await generateBattle(master, start + offset);
