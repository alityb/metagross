...
def observation_history(observation_space) -> dict[str, object]:
    """Snapshot the mutable public-history fields used by R1 observations."""
    ...
    if any(not hasattr(current, field) for field in required):
        raise RuntimeError("R1 observation space does not expose public history")
    return {
...
def player_information_state(battle, state) -> dict[str, object]:
    """Serialize only own-private and currently public opponent information."""
    from metamon.interface import UniversalMove, UniversalPokemon

    def team_rows(team, active_pokemon) -> list[dict[str, object]]:
        ...
        for slot, pokemon in team.items():
            rows.append(
                {
                    "slot": str(slot),
                    "active": active_species is not None
                    and norm(pokemon.species) == active_species,
                    "pokemon": asdict(UniversalPokemon.from_Pokemon(pokemon)),
                    "previous_move": asdict(UniversalMove.from_Move(pokemon.previous_move)),
                }
            )
        if active_species is not None and sum(bool(row["active"]) for row in rows) != 1:
...
class BattleSession:
    ...
    def feed_line(self, line: str) -> None:
        if self.server.dual_r1_capture:
    ...
    def compute_priors(
        ...
        if self.server.trajectory_mode == "legacy-stateless":
            ...
            # History truncation ablation (redundancy hypothesis): cap the
            # causal window to the last K request-steps. Absent env =
            # byte-identical serving; set-but-malformed = fail closed; active
            # = one-time observable log line. Truncated windows reuse the same
            # (start, time_offset) mechanics as the max_seq_len overflow path,
            # so positional timing stays in-distribution.
            effective_seq_len = max_seq_len
            truncate_raw = os.environ.get("METAGROSS_HISTORY_TRUNCATE_STEPS")
            if truncate_raw:
                try:
                    truncate_steps = int(truncate_raw)
                except ValueError as exc:
                    raise RuntimeError(
                        f"METAGROSS_HISTORY_TRUNCATE_STEPS is set but invalid: {exc}"
                    )
                if truncate_steps < 2:
                    raise RuntimeError(
                        "METAGROSS_HISTORY_TRUNCATE_STEPS must be >= 2"
                    )
                effective_seq_len = min(max_seq_len, truncate_steps)
                if not getattr(self.server, "_truncate_logged", False):
                    print(
                        f"HISTORY_TRUNCATION ACTIVE last_{effective_seq_len}_steps",
                        flush=True,
                    )
                    self.server._truncate_logged = True
        ...
        dump_row = None
        if self.server.dump_path or self.server.dual_r1_capture:
            ...
            # durably recorded.
            dump_row = {
                "schema": 6 if self.server.dual_r1_capture else 5,
                "tag": self.tag,
                ...
                dump_row.update(
                    {
                        ...
                        "opponent_username": getattr(
                            self.battle, "opponent_username", None
                        ),
                        "protocol_prefix": list(self.protocol_lines),
                        "player_information_state": player_information_state(
                            self.battle, state
                        ),
                        "player_observation_history": observation_history(
                            self.obs_space
                        ),
                        "continuation_observation_history": observation_history(
                            self.obs_space
                        ),
                    }
                )
                dump_row["trajectory"]["observation_rows"] = {
                    key: value.tolist() for key, value in trajectory_obs.items()
                }
            line = json.dumps(dump_row, separators=(",", ":")) + "\n"
        if self.server.dump_path and dump_row is not None:
        ...
        if dump_row is not None and self.server.dual_r1_capture:
            response["r1_policy_snapshot"] = dump_row
        self.cached_rqid = rqid
        self.cached_request_sha256 = expected_request_sha256
        self.cached_response = response
        return dict(response)
...
class PriorServer:
    def __init__(self, args):
        ...
        self.dual_r1_capture = os.environ.get("METAGROSS_DUAL_R1_CAPTURE") == "1"
