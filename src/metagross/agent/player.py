...
from metagross.belief import BeliefStateModule
from metagross.search import MCTSEngine
from metagross.model.checkpoint import load_checkpoint, new_model_from_pool
from metagross.model.state import build_vocabulary, encode_state
from metagross.rlm import RLMStrategist
from metagross.rlm.strategist import NullRLMStrategist
...
class AlphaPokemonCore:
    ...
    def choose_action_index(self, battle: Any) -> int:
        ...
        base_policy, _value = self.pokenet.policy_value(state)
        # 2. RLM strategic reasoning.
        rlm_out = self.rlm.assess(log=self.full_log, state=battle, pool=self.pool, base_policy=base_policy[0].detach().cpu().tolist())
        # 3. Merge RLM posterior.
        self.belief.refine(rlm_out.refined_belief)
        # 4-5. Sample configs and run root RLM-augmented MCTS.
